...
rule rosella_binning:
    ...
    shell:
        ...
        if [ $? -ne 0 ]; then
            echo "WARNING: Rosella binning failed, creating empty outputs"
...
rule metacoag_binning:
    ...
    shell:
        ...
        if [ $? -ne 0 ]; then
            echo "WARNING: MetaCoAG binning failed, creating empty output"
            exit 0
        fi
        """
...
rule metabat_binning:
    ...
    shell:
        ...
        if [ $? -ne 0 ]; then
            echo "WARNING: MetaBAT binning failed, creating empty outputs"
            mkdir -p metabat/bins
            touch prokarya_contigs.fasta.depth.txt
            exit 0
        fi
...
rule maxbin_binning:
    ...
    shell:
        ...
        if [ $? -ne 0 ]; then
            echo "WARNING: MaxBin binning failed, creating empty output"
            mkdir -p {output.bins_dir}
            exit 0
        fi
...
rule semibin_binning:
    ...
    shell:
        ...
        if [ $? -ne 0 ]; then
            echo "WARNING: SemiBin binning failed, creating empty output"
            mkdir -p {output.bins_dir}
            exit 0
        fi
...
rule scaffold_all_bins:
    ...
    shell:
        """
        set +e  # Don't exit on error for individual bins
        ...
        for bin_file in "$REFINED_DIR_ABS"/*.fna; do
            ...
            # Run ntLink with absolute paths (allow failure)
            ntLink scaffold gap_fill target="$bin_abs" reads="$READS_ABS" t={threads} || true
            ...
            if [ -f *.ntLink.scaffolds.gap_fill.fa ]; then
                ...
                echo "WARNING: ntLink failed for $bin_name, using original bin"
...
rule polish_all_bins:
    input:
        ...
        reads = "{outdir}/QC.fastq",
        dastool_bins = "{outdir}/DAStool_refined/bins"
    output:
    ...
    shell:
        ...
        for scaffolded_file in {params.outdir}/scaffolded_bins/*.fna; do
            ...
            if [ $? -ne 0 ]; then
                echo "WARNING: Polishing failed for $bin_name (minimap2 round 1), using unpolished bin"
                cp {input.dastool_bins}/${{bin_name}}.fna {params.outdir}/polished_bins/${{bin_name}}_polished.fasta 2>/dev/null || \
                cp "$scaffolded_file" {params.outdir}/polished_bins/${{bin_name}}_polished.fasta
                continue
            fi
            ...
            if [ $? -ne 0 ] || [ ! -s {params.outdir}/racon_temp/${{bin_name}}_racon1.fasta ]; then
                echo "WARNING: Polishing failed for $bin_name (racon round 1), using unpolished bin"
                cp {input.dastool_bins}/${{bin_name}}.fna {params.outdir}/polished_bins/${{bin_name}}_polished.fasta 2>/dev/null || \
                cp "$scaffolded_file" {params.outdir}/polished_bins/${{bin_name}}_polished.fasta
                rm {params.outdir}/racon_temp/${{bin_name}}_aln1.sam 2>/dev/null || true
                continue
            fi
            ...
            if [ $? -ne 0 ]; then
                echo "WARNING: Polishing failed for $bin_name (minimap2 round 2), using unpolished bin"
                cp {input.dastool_bins}/${{bin_name}}.fna {params.outdir}/polished_bins/${{bin_name}}_polished.fasta 2>/dev/null || \
                cp "$scaffolded_file" {params.outdir}/polished_bins/${{bin_name}}_polished.fasta
                continue
            fi
            ...
            if [ $? -ne 0 ] || [ ! -s {params.outdir}/polished_bins/${{bin_name}}_polished.fasta ]; then
                echo "WARNING: Polishing failed for $bin_name (racon round 2), using unpolished bin"
                cp {input.dastool_bins}/${{bin_name}}.fna {params.outdir}/polished_bins/${{bin_name}}_polished.fasta 2>/dev/null || \
                cp "$scaffolded_file" {params.outdir}/polished_bins/${{bin_name}}_polished.fasta
            fi

            rm {params.outdir}/racon_temp/${{bin_name}}_aln2.sam 2>/dev/null || true
        done
